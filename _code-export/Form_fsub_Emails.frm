...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "Form_fsub_Emails"
...
Private Sub Form_BeforeInsert(Cancel As Integer)
    Cancel = g_Model.Db_isReadOnly
    If Cancel Then Exit Sub
    Cancel = MsgBox("OK to bypass the Mailstage and register a new empty Email?", vbOKCancel + vbQuestion, "Bypass Mailstage?") <> vbOK
    If Cancel Then Exit Sub
    SetDefaults_Create
End Sub
...
Private Sub Form_Current()
    Form_frm_Emails.Refresh_Captions
End Sub
...
Private Sub Form_Open(Cancel As Integer)
    Query_Refresh "quni_" & suffix, std_Sql.Emails_quni
...
Private Sub Refresh_ContextMenu()
    ...
    With New cls_Permissor
        ...
        If .Can_ReadFs Then
            AddCustomButton cbar, "Open (SHARED)", "Email_OpenWIP", faceId_WorkInProgress
            AddCustomButton cbar, "Latest Snapshot", "Email_OpenLatestSnapshot", faceId_SnapshotLatest
            AddCustomButton cbar, "All Snapshots", "Email_OpenSnapshots", faceId_SnapshotAll
        End If
        AddCustomButton cbar, "Comments", "Email_Comments", faceId_Comments
        ...
        If .Can_Attach Then AddCustomSubButton(sub_Edit, "Attach => " & .scheme, "Email_Attach", faceId_Attach).BeginGroup = True
        If .Can_Detach Then AddCustomSubButton(sub_Edit, "Detach <= " & .scheme, "Email_Detach", faceId_Detach).BeginGroup = True
        If .Can_TakeSnapshot Then AddCustomSubButton sub_Edit, "Take Snapshot", "Email_CreateSnapshot", faceId_SnapshotSave
        If .Can_Unfreeze Then AddCustomSubButton(sub_Edit, "Unfreeze", "Email_IsFrozen", faceId_Unfreeze).BeginGroup = True
        If .Can_Freeze Then AddCustomSubButton(sub_Edit, "Freeze", "Email_IsFrozen", faceId_Freeze).BeginGroup = True
        If .Can_Undelete Then AddCustomSubButton sub_Edit, "Undelete", "Email_Undelete", faceId_Undelete
